...
void ARPAPredictor::loadVocabulary()
{
	...
	logger << DEBUGD << "["<<row<<"] -> "<< code<<endl;

	code++;
    }

    logger << DEBUGD << "Loaded "<<code<<" words from vocabulary" <<endl;
...
void ARPAPredictor::createARPATable()
{
	...
	if( startData == true && currOrder == 0)
	{
		...
		logger << DEBUGD << "tot unigram = "<<unigramTot<<endl;
		continue;
	    }
		...
		logger << DEBUGD << "tot bigram = "<<bigramTot<<endl;
		continue;
	    }
		...
		logger << DEBUGD << "tot trigram = "<<trigramTot<<endl;
		continue;
	    }
	}
    ...
    logger << DEBUGD << "loaded unigrams: "<< unigramCount << endl;
    logger << DEBUGD << "loaded bigrams: " << bigramCount << endl;
    logger << DEBUGD << "loaded trigrams: "<< trigramCount << endl;
}
...
void ARPAPredictor::addUnigram(std::string row)
{
	...
	logger << DEBUGD << "adding unigram ["<<wd1Str<< "] -> "<<logProb<<" "<<logAlfa<<endl;
    }
...
void ARPAPredictor::addBigram(std::string row)
{
	...
	logger << DEBUGD << "adding bigram ["<<wd1Str<< "] ["<<wd2Str<< "] -> "<<logProb<<" "<<logAlfa<<endl;
    }
...
void ARPAPredictor::addTrigram(std::string row)
{
	...
	logger << DEBUGD << "adding trigram ["<<wd1Str<< "] ["<<wd2Str<< "] ["<<wd3Str<< "] -> "<<logProb <<endl;
...
Prediction ARPAPredictor::predict(const size_t max_partial_prediction_size, const char** filter) const
{
    ...
    logger << DEBUGD << "["<<wd1Str<<"]"<<" ["<<wd2Str<<"] "<<"["<<prefix<<"]"<<endl;
...
{
    logger << DEBUGD << "computing P( ["<<vocabDecode.find(wd3)->second<< "] | ["<<vocabDecode.find(wd1)->second<<"] ["<<vocabDecode.find(wd2)->second<<"] )"<<endl;

    //trigram exist
    std::map<TrigramKey,float>::const_iterator trigramIt =trigramMap.find(TrigramKey(wd1,wd2,wd3));
    if(trigramIt!=trigramMap.end())
    {
	logger << DEBUGD << "trigram ["<<vocabDecode.find(wd1)->second<< "] ["<<vocabDecode.find(wd2)->second<< "] ["<<vocabDecode.find(wd3)->second<< "] exists" <<endl;
	logger << DEBUGD << "returning "<<trigramIt->second <<endl;
    ...
    {
	logger << DEBUGD << "bigram ["<<vocabDecode.find(wd1)->second<< "] ["<<vocabDecode.find(wd2)->second<< "] exists" <<endl;
	float prob = bigramIt->second.logAlfa + computeBigramBackoff(wd2,wd3);
	logger << DEBUGD << "returning "<<prob<<endl;
	return prob;
    }

    //else
    logger << DEBUGD << "no bigram w1,w2 exist" <<endl;
...
{
    logger << DEBUGD << "learn() method called" << endl;
    logger << DEBUGD << "learn() method exited" << endl;
}

void ARPAPredictor::update (const Observable* var)
{
    logger << DEBUGD << "About to invoke dispatcher: " << var->get_name () << " - " << var->get_value() << endl;

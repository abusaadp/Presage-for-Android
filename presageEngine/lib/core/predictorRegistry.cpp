...
{
    logger << DEBUGD << "Removing predictor: " << predictor_name << endl;
	...
	if ((*it)->getName() == predictor_name)
	{
	    ...
	    logger << DEBUGD << "Removed predictor: " << predictor_name << endl;
	}
	else
	{
	    ++it;
...
void PredictorRegistry::removePredictors()
{
	...
	logger << DEBUGD << "Removing predictor: " << predictors[i]->getName() << endl;
	delete predictors[i];
    }
    predictors.clear();
}
...
{
    logger << DEBUGD << "About to invoke dispatcher: " << variable->get_name () << " - " << variable->get_value() << endl;

...
std::vector<std::string> Selector::select( Prediction p )
{
	...
	logger << DEBUGD << "Added token to selector consideration set: " << token << endl;
    }
	
    // check whether user has not moved on to a new word
    if (contextTracker->contextChange()) {
	logger << DEBUGD << "Context change detected." << endl;
	clearSuggestedWords();
    } else {
	logger << DEBUGD << "No context change detected." << endl;
    }
...
void Selector::updateSuggestedWords( const std::vector<std::string>& v )
{
    ...
    while( i != v.end() ) {
	logger << DEBUGD << "Adding token to suggested token set: " << *i << endl;
	suggestedWords.insert( *i );
	i++;
    }

    logger << DEBUGD << "Suggested words: ";
...
{
    logger << DEBUGD << "Clearing previously suggested tokens set." << endl;
...
void Selector::repetitionFilter( std::vector<std::string>& v )
{
	...
	if( suggestedWords.find( *i ) == suggestedWords.end() ) {
	    temp.push_back( *i );
	    logger << DEBUGD << "Token passed repetition filter: " << *i << endl;
	} else {
	    logger << DEBUGD << "Token failed repetition filter: " << *i << endl;
	}
    }
...
{
    logger << DEBUGD << "update(" << variable->get_name () << ") called" << endl;

...
void SmoothedNgramPredictor::set_deltas (const std::string& value)
{
    ...
    while (ss_deltas >> delta) {
        logger << DEBUGD << "Pushing delta: " << delta << endl;
...
unsigned int SmoothedNgramPredictor::count(const std::vector<std::string>& tokens, int offset, int ngram_size) const
{
	...
	logger << DEBUGD << "count ngram: " << ngram_to_string (ngram) << " : " << result << endl;
    } else {
	result = db->getUnigramCountsSum();
	logger << DEBUGD << "unigram counts sum: " << result << endl;
    }
...
Prediction SmoothedNgramPredictor::predict(const size_t max_partial_prediction_size, const char** filter) const
{
	...
	logger << DEBUGD << "Cached tokens[" << cardinality - 1 - i << "] = " << tokens[cardinality - 1 - i] << endl;
    }
    ...
    for (size_t k = cardinality; (k > 0 && prefixCompletionCandidates.size() < max_partial_prediction_size); k--) {
        logger << DEBUGD << "Building partial prefix completion table of cardinality: " << k << endl;
	...
	if (logger.shouldLog()) {
	    logger << DEBUGD << "prefix_ngram: ";
	    for (size_t r = 0; r < prefix_ngram.size(); r++) {
		logger << DEBUGD << prefix_ngram[r] << ' ';
	    }
	    logger << DEBUGD << endl;
	}
	...
	if (logger.shouldLog()) {
	    logger << DEBUGD << "partial prefixCompletionCandidates" << endl
	           << DEBUGD << "----------------------------------" << endl;
	    for (size_t j = 0; j < partial.size(); j++) {
		for (size_t k = 0; k < partial[j].size(); k++) {
		    logger << DEBUGD << partial[j][k] << " ";
		}
		logger << endl;
	    }
	}

        logger << DEBUGD << "Partial prefix completion table contains " << partial.size() << " potential completions." << endl;
    ...
    if (logger.shouldLog()) {
	logger << DEBUGD << "prefixCompletionCandidates" << endl
	       << DEBUGD << "--------------------------" << endl;
	for (size_t j = 0; j < prefixCompletionCandidates.size(); j++) {
	    logger << DEBUGD << prefixCompletionCandidates[j] << endl;
	}
    }
	...
	logger << DEBUGD << "------------------" << endl;
	logger << DEBUGD << "w_i: " << tokens[cardinality - 1] << endl;
	...
	for (int k = 0; k < cardinality; k++) {
	    ...
	    logger << DEBUGD << "numerator:   " << numerator << endl;
	    logger << DEBUGD << "denominator: " << denominator << endl;
	    logger << DEBUGD << "frequency:   " << frequency << endl;
	    logger << DEBUGD << "delta:       " << lDeltas[k] << endl;
        ...
        logger << DEBUGD << "____________" << endl;
        logger << DEBUGD << "probability: " << probability << endl;
...
void SmoothedNgramPredictor::learn(const std::vector<std::string>& change)
{
	    ...
	    change.front() == contextTracker->getToken(change.size()))
	{
		...
		logger << DEBUGD << "Adding extra token: " << extra_token << endl;
...
void SmoothedNgramPredictor::check_learn_consistency(const Ngram& ngram) const
{
	...
	if (count(ngram, -i, size - i) > count(ngram, -(i + 1), size - (i + 1))) {
	    ...
	    logger << DEBUGD << "i: " << i << " | offset: " << offset << " | sub_ngram_size: " << sub_ngram_size << endl;
	    ...
	    logger << DEBUGD << "consistency adjusted" << endl;
	}
    }
}
